...
public:	
	...
	// Takes the item to represent and creates the pickup from it
	void InitializePickup(const TSubclassOf<class UItem> ItemClass, const int32 Quantity);

	// Align pickups rotation with the ground rotation
	UFUNCTION(BlueprintImplementableEvent)
	void AlignWithGround();

	// This is used as a template to create the pickup when it is spawned in
